...
import (
	...
	wasm "github.com/comdex-official/comdex/app/wasm"
	bindings "github.com/comdex-official/comdex/app/wasm/bindings"
	"github.com/comdex-official/comdex/x/tokenfactory/types"
)

func TestCreateDenom(t *testing.T) {
	actor := RandomAccountAddress()
	comdexapp, ctx := SetupCustomApp(t, actor)
	...
	fundAccount(t, *ctx, comdexapp, actor, actorAmount)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			_, gotErr := wasm.PerformCreateDenom(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			comdexapp, ctx := SetupCustomApp(t, tokenCreator)

			// Fund actor with 100 base denom creation fees
			actorAmount := sdk.NewCoins(sdk.NewCoin(types.DefaultParams().DenomCreationFee[0].Denom, types.DefaultParams().DenomCreationFee[0].Amount.MulRaw(100)))
			fundAccount(t, *ctx, comdexapp, tokenCreator, actorAmount)

			_, err := wasm.PerformCreateDenom(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasm.ChangeAdmin(comdexapp.TokenFactoryKeeper, *ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	comdexapp, ctx := SetupCustomApp(t, creator)
	...
	fundAccount(t, *ctx, comdexapp, creator, tokenCreationFeeAmt)

	// Create denoms for valid mint tests
	validDenom := bindings.CreateDenom{
		Subdenom: "MOON",
	}
	_, err := wasm.PerformCreateDenom(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	_, err = wasm.PerformCreateDenom(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	// lucky was broke
	balances := comdexapp.BankKeeper.GetAllBalances(*ctx, lucky)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasm.PerformMint(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasm.PerformMint(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasm.PerformMint(comdexapp.TokenFactoryKeeper, comdexapp.BankKeeper, *ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasm.PerformBurn(comdexapp.TokenFactoryKeeper, *ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
