...
func TestCreateDenomMsg(t *testing.T) {
	...
	reflect := instantiateReflectContract(t, *ctx, comdexapp, lucky)
	...
	fundAccount(t, *ctx, comdexapp, reflect, reflectAmount)

	msg := bindings.TokenFactoryMsg{CreateDenom: &bindings.CreateDenom{
		Subdenom: "SUN",
	}}
	err := executeCustom(t, *ctx, comdexapp, reflect, lucky, msg, sdk.Coin{})
	require.NoError(t, err)

	// query the denom and see if it matches
	query := bindings.ContractQuery{
	...
	queryCustom(t, *ctx, comdexapp, reflect, query, &resp)
...
func TestForceTransfer(t *testing.T) {
	...
	balances = comdexapp.BankKeeper.GetAllBalances(*ctx, rcpt)
	require.Len(t, balances, 1)
	coin := balances[0]
...
func TestBurnMsg(t *testing.T) {
	...
	luckyBalance := comdexapp.BankKeeper.GetAllBalances(*ctx, lucky)
	err = comdexapp.BankKeeper.SendCoins(*ctx, lucky, reflect, luckyBalance)
	require.NoError(t, err)
...
func executeCustom(t *testing.T, ctx sdk.Context, comdexapp *app.App, contract sdk.AccAddress, sender sdk.AccAddress, msg bindings.TokenFactoryMsg, funds sdk.Coin) error { //nolint:unparam // funds is always nil but could change in the future.
	...
	contractKeeper := keeper.NewDefaultPermissionKeeper(comdexapp.WasmKeeper)
