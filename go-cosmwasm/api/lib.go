...
import (
	...
	v1types "github.com/comdex-official/comdex/go-cosmwasm/types/v1"
...
// 	res, err := C.migrate(cache.ptr, id, p, m, u64(gasLimit), &gasUsed, &errmsg, s, adminBuffer, adminProofBuffer)
...
// 	res, err := C.update_admin(cache.ptr, id, p, a, q, u64(gasLimit), &errmsg, s, currentAdminBuffer, currentAdminProofBuffer, newAdminBuffer)
// 	if err != nil && err.(syscall.Errno) != C.ErrnoValue_Success {
// 		return nil, errorWithMessage(err, errmsg)
// 	}
// 	return receiveVector(res), nil
...
// 	res, err := C.instantiate(cache.ptr, id, p, m, a, q, u64(gasLimit), &gasUsed, &errmsg, s, adminBuffer)
// 	if err != nil && err.(syscall.Errno) != C.ErrnoValue_Success {
// 		// Depending on the nature of the error, `gasUsed` will either have a meaningful value, or just 0.
// 		return nil, uint64(gasUsed), errorWithMessage(err, errmsg)
// 	}
...
// 	res, err := C.handle(cache.ptr, id, p, m, a, q, u64(gasLimit), &gasUsed, &errmsg, s, u8(handleType))
// 	if err != nil && err.(syscall.Errno) != C.ErrnoValue_Success {
// 		// Depending on the nature of the error, `gasUsed` will either have a meaningful value, or just 0.
// 		return nil, uint64(gasUsed), errorWithMessage(err, errmsg)
// 	}
...
// 	res, err := C.query(cache.ptr, id, p, m, a, q, u64(gasLimit), &gasUsed, &errmsg)
// 	if err != nil && err.(syscall.Errno) != C.ErrnoValue_Success {
// 		// Depending on the nature of the error, `gasUsed` will either have a meaningful value, or just 0.
// 		return nil, uint64(gasUsed), errorWithMessage(err, errmsg)
// 	}
