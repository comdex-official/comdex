...
import (
	...
	comdextypes "github.com/comdex-official/comdex/types"
...
func (im *IBCModule) OnRecvPacket(
	...
	if err := ValidateReceiverAddress(packet); err != nil {
		return comdextypes.NewEmitErrorAcknowledgement(ctx, types.ErrBadMessage, err.Error())
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "rate limit exceeded") {
			return comdextypes.NewEmitErrorAcknowledgement(ctx, types.ErrRateLimitExceeded)
		}
		fullError := sdkerrors.Wrap(types.ErrContractError, err.Error())
		return comdextypes.NewEmitErrorAcknowledgement(ctx, fullError)
	}
...
func (im *IBCModule) OnAcknowledgementPacket(
	...
	if comdextypes.IsAckError(acknowledgement) {
