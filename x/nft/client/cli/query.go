...
$ %s query nft supply [denom-id]`, version.AppName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := queryClient.QuerySupply(context.Background(), &types.QuerySupplyRequest{
				DenomId: denomId,
				Owner:   owner.String(),
			})
			if err != nil {
...
func GetCmdQueryOwner() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			resp, err := queryClient.QueryOwnerNFTs(context.Background(), &types.QueryOwnerNFTsRequest{
				DenomId:    denomID,
				Owner:      args[0],
...
$ %s query nft collections <denom-id>`, version.AppName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			resp, err := queryClient.QueryCollection(context.Background(), &types.QueryCollectionRequest{
				DenomId:    denomId,
				Pagination: pagination,
			})
			if err != nil {
...
$ %s query nft denoms`, version.AppName)),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			resp, err := queryClient.QueryDenoms(
				context.Background(),
...
$ %s query nft denom <denom-id>`, version.AppName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			resp, err := queryClient.QueryDenom(context.Background(), &types.QueryDenomRequest{
				DenomId: denomId,
			})
			if err != nil {
				return err
...
$ %s query nft asset <denom> <nft-id>`, version.AppName)),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			resp, err := queryClient.QueryNFT(context.Background(), &types.QueryNFTRequest{
				DenomId: denomId,
				Id:      nftId,
			})
			if err != nil {
