...
func GetQueryCmd(queryRoute string) *cobra.Command {
	// Group bonding queries under a subcommand
	cmd := &cobra.Command{
	...
	cmd.AddCommand(
		...
		GetCmdSyntheticBondingsByBondingID(),
	)
...
Example:
$ %s query bonding module-balance
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding module-locked-amount
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-unlockable-coins <address>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-unlocking-coins <address>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-coins <address>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-pastime <address> <timestamp>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-pastime-not-unlocking <address> <timestamp>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-pastime-denom <address> <timestamp> <denom>
`,
				version.AppName,
			),
		),
...
// GetCmdSyntheticBondingsByBondingID returns synthetic bondings by bonding id.
func GetCmdSyntheticBondingsByBondingID() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "synthetic-bondings-by-lock-id <id>",
		Short: "Query synthetic bondings by bonding id",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query synthetic bondings by bonding id.

Example:
$ %s query bonding synthetic-bondings-by-lock-id <id>
`,
				version.AppName,
			),
		),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := queryClient.SyntheticBondingsByBondingID(cmd.Context(), &types.SyntheticBondingsByBondingIDRequest{LockId: id})
			if err != nil {
				return err
			}
...
Example:
$ %s query bonding account-locked-longer-duration <address> <duration>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-longer-duration-not-unlocking <address> <duration>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding account-locked-pastime <address> <duration> <denom>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding total-locked-of-denom <denom>
`,
				version.AppName,
			),
		),
...
Example:
$ %s query bonding output-all-locks <max lock ID>
`,
				version.AppName,
			),
		),
