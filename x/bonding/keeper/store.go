...
func syntheticLockStoreKey(lockID uint64, synthDenom string) []byte {
	return combineKeys(combineKeys(types.KeyPrefixSyntheticBonding, sdk.Uint64ToBigEndian(lockID)), []byte(synthDenom))
}
...
func (k Keeper) GetLockByID(ctx sdk.Context, lockID uint64) (*types.PeriodLock, error) {
	...
	if !store.Has(lockKey) {
		return nil, sdkerrors.Wrap(types.ErrBondingNotFound, fmt.Sprintf("lock with ID %d does not exist", lockID))
	}
