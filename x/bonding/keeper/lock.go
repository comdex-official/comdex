...
import (
	...
	"github.com/comdex-official/comdex/store"
...
func (k Keeper) addTokensToLock(ctx sdk.Context, lock *types.PeriodLock, coins sdk.Coins) error {
	...
	if err == nil {
		for _, synthlock := range k.GetAllSyntheticBondingsByBonding(ctx, lock.ID) {
...
// removeTokensFromLock is called by bonding slash function - called by superfluid module only.
func (k Keeper) removeTokensFromLock(ctx sdk.Context, lock *types.PeriodLock, coins sdk.Coins) error {
	// TODO: Handle 100% slash eventually, not needed for comdex codebase atm.
	...
	// increase synthetic bonding's accumulation store
	synthLocks := k.GetAllSyntheticBondingsByBonding(ctx, lock.ID)

	// Note: since synthetic bonding deletion is using native bonding's coins to reduce accumulation store
	// all the synthetic bondings' accumulation should be decreased
...
func (k Keeper) BeginUnlock(ctx sdk.Context, lockID uint64, coins sdk.Coins) error {
	...
	if k.HasAnySyntheticBondings(ctx, lock.ID) {
		return fmt.Errorf("cannot BeginUnlocking a lock with synthetic bonding")
	}
