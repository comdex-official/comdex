...
// Querier defines a wrapper around the x/bonding keeper providing gRPC method
// handlers.
type Querier struct {
	Keeper
}
...
// SyntheticBondingsByBondingID returns synthetic bondings by native bonding id.
func (q Querier) SyntheticBondingsByBondingID(goCtx context.Context, req *types.SyntheticBondingsByBondingIDRequest) (*types.SyntheticBondingsByBondingIDResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	synthLocks := q.Keeper.GetAllSyntheticBondingsByBonding(ctx, req.LockId)
	return &types.SyntheticBondingsByBondingIDResponse{SyntheticLocks: synthLocks}, nil
}
