...
// MergeBondingsForSimilarDurations iterates through every account. For each account,
// it combines all bondings it has at a similar duration (to be defined in a bit).
// It will delete every existing bonding for that account, and make at most, a single new bonding per
// "base duration", denom pair.
// If a bonding is far from any base duration, we don't change anything about it.
// We define a bonding length as a "Similar duration to base duration D", if:
// D <= bonding length <= D + durationDiff.
func MergeBondingsForSimilarDurations(
	ctx sdk.Context,
	k Keeper,
