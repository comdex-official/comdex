...
Bonding keeper provides utility functions to store lock queues and query locks.

```go
// Keeper is the interface for bonding module keeper
type Keeper interface {
    ...
    GetSyntheticBonding(ctx sdk.Context, lockID uint64, suffix string) (*types.SyntheticLock, error)
    GetAllSyntheticBondingsByBonding(ctx sdk.Context, lockID uint64) []types.SyntheticLock
    GetAllSyntheticBondings(ctx sdk.Context) []types.SyntheticLock
    // CreateSyntheticBonding create synthetic bonding with lock id and denom suffix
    CreateSyntheticBonding(ctx sdk.Context, lockID uint64, suffix string, unlockDuration time.Duration) error
    // DeleteSyntheticBonding delete synthetic bonding with lock id and suffix
    DeleteSyntheticBonding(ctx sdk.Context, lockID uint64, suffix string) error
    DeleteAllMaturedSyntheticLocks(ctx sdk.Context)
```

# Lock Admin Keeper

Bonding admin keeper provides god privilege functions to remove tokens from locks and create new locks.
...
// and form a new pool with the old parameters but if they still had 2 months of bonding left,
// their liquidity still needs to be 2 month bonding-ed, just in the new pool
