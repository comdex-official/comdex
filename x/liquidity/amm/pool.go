...
func CreateBasicPool(rx, ry sdk.Int) (*BasicPool, error) {
	...
	p := sdk.NewDec(rx.Int64()).Quo(sdk.NewDec(ry.Int64()))
...
func (pool *BasicPool) Price() sdk.Dec {
	...
	return sdk.NewDec(pool.rx.Int64()).Quo(sdk.NewDec(pool.ry.Int64()))
}
...
func (pool *BasicPool) BuyAmountOver(price sdk.Dec, _ bool) (amt sdk.Int) {
	...
	dx := sdk.NewDec(pool.rx.Int64()).Sub(price.MulInt(pool.ry))
	if !dx.IsPositive() {
		return zeroInt
	}
	utils.SafeMath(func() {
...
func (pool *BasicPool) SellAmountUnder(price sdk.Dec, _ bool) (amt sdk.Int) {
	...
	amt = sdk.NewDec(pool.ry.Int64()).Sub(sdk.NewDec(pool.rx.Int64()).QuoRoundUp(price)).TruncateInt()
	if !amt.IsPositive() {
		return zeroInt
	}
	return
...
func (pool *BasicPool) BuyAmountTo(price sdk.Dec) (amt sdk.Int) {
	...
	sqrtRx := utils.DecApproxSqrt(sdk.NewDec(pool.rx.Int64()))
	sqrtRy := utils.DecApproxSqrt(sdk.NewDec(pool.ry.Int64()))
	sqrtPrice := utils.DecApproxSqrt(price)
	dx := sdk.NewDec(pool.rx.Int64()).Sub(sqrtPrice.Mul(sqrtRx.Mul(sqrtRy))) // dx = rx - sqrt(P * rx * ry)
	if !dx.IsPositive() {
		return zeroInt
	}
	utils.SafeMath(func() {
...
func (pool *BasicPool) SellAmountTo(price sdk.Dec) (amt sdk.Int) {
	...
	sqrtPrice := utils.DecApproxSqrt(price)
	// dy = ry - sqrt(rx * ry / P)
	amt = sdk.NewDec(pool.ry.Int64()).Sub(sqrtRx.Mul(sqrtRy).Quo(sqrtPrice)).TruncateInt()
	if !amt.IsPositive() {
		return zeroInt
	}
	return
...
func NewRangedPool(rx, ry, ps sdk.Int, minPrice, maxPrice sdk.Dec) *RangedPool {
	...
	return &RangedPool{
		...
		transX:   transX,
		transY:   transY,
		xComp:    sdk.NewDec(rx.Int64()).Add(transX),
		yComp:    sdk.NewDec(ry.Int64()).Add(transY),
	}
}
...
func CreateRangedPool(x, y sdk.Int, minPrice, maxPrice, initialPrice sdk.Dec) (pool *RangedPool, err error) {
	...
	default: // normal pool
		...
		xDec, yDec := sdk.NewDec(x.Int64()), sdk.NewDec(y.Int64())
...
func (pool *RangedPool) SetBalances(rx, ry sdk.Int, derive bool) {
	...
	pool.rx = rx
	pool.ry = ry
	pool.xComp = sdk.NewDec(pool.rx.Int64()).Add(pool.transX)
	pool.yComp = sdk.NewDec(pool.ry.Int64()).Add(pool.transY)
}
...
func (pool *RangedPool) BuyAmountOver(price sdk.Dec, _ bool) (amt sdk.Int) {
	...
	if !dx.IsPositive() {
		return zeroInt
	} else if dx.GT(sdk.NewDec(pool.rx.Int64())) {
		dx = sdk.NewDec(pool.rx.Int64())
	}
	utils.SafeMath(func() {
		amt = dx.QuoTruncate(origPrice).TruncateInt() // dy = dx / P
		if amt.GT(MaxCoinAmount) {
...
func (pool *RangedPool) BuyAmountTo(price sdk.Dec) (amt sdk.Int) {
	...
	dx := sdk.NewDec(pool.rx.Int64()).Sub(sqrtPrice.Mul(sqrtXComp.Mul(sqrtYComp)).Sub(pool.transX))
	if !dx.IsPositive() {
		return zeroInt
	} else if dx.GT(sdk.NewDec(pool.rx.Int64())) {
		dx = sdk.NewDec(pool.rx.Int64())
...
func (pool *RangedPool) SellAmountTo(price sdk.Dec) (amt sdk.Int) {
	...
	amt = sdk.NewDec(pool.ry.Int64()).Sub(sqrtXComp.Mul(sqrtYComp).QuoRoundUp(sqrtPrice).Sub(pool.transY)).TruncateInt()
	if amt.GT(pool.ry) {
		amt = pool.ry
	}
	if !amt.IsPositive() {
...
func Deposit(rx, ry, ps, x, y sdk.Int) (ax, ay, pc sdk.Int) {
	...
	utils.SafeMath(func() {
		rx, ry := sdk.NewDec(rx.Int64()), sdk.NewDec(ry.Int64())
		ps := sdk.NewDec(ps.Int64())

		// pc = floor(ps * min(x / rx, y / ry))
		var ratio sdk.Dec
		switch {
		case rx.IsZero():
			ratio = sdk.NewDec(y.Int64()).QuoTruncate(ry)
		case ry.IsZero():
			ratio = sdk.NewDec(x.Int64()).QuoTruncate(rx)
		default:
			ratio = sdk.MinDec(
				sdk.NewDec(x.Int64()).QuoTruncate(rx),
				sdk.NewDec(y.Int64()).QuoTruncate(ry),
			)
		}
		pc = ps.MulTruncate(ratio).TruncateInt()

		mintProportion := sdk.NewDec(pc.Int64()).Quo(ps)             // pc / ps
...
func Withdraw(rx, ry, ps, pc sdk.Int, feeRate sdk.Dec) (x, y sdk.Int) {
	...
	utils.SafeMath(func() {
		proportion := sdk.NewDec(pc.Int64()).QuoTruncate(sdk.NewDec(ps.Int64()))                             // pc / ps
...
func DeriveTranslation(rx, ry sdk.Int, minPrice, maxPrice sdk.Dec) (transX, transY sdk.Dec) {
	...
	rxDec, ryDec := sdk.NewDec(rx.Int64()), sdk.NewDec(ry.Int64())
	sqrtM := sqrt(minPrice)
	sqrtL := sqrt(maxPrice)
