...
/*func (k *Keeper) SetExternalRewardsLockersCounter(ctx sdk.Context, AppId uint64) {
	var (
		store = k.Store(ctx)
		key   = types.ExternalRewardsLockersCounter(AppId)
		value = k.cdc.MustMarshal(
			&protobuftypes.UInt64Value{
				Value: AppId,
			},
...
func (k *Keeper) GetExternalRewardsLockersCounter(ctx sdk.Context, Id uint64) (count uint64) {
	var (
		store = k.Store(ctx)
		key   = types.ExternalRewardsLockersCounter(Id)
		value = store.Get(key)
	)
	if value == nil {
		return count
