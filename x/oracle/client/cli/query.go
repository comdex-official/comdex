...
import (
	...
	"github.com/comdex-official/comdex/types/cli"
...
$ comdex query oracle aggregate-votes

Or, you can filter with voter address

$ comdex query oracle aggregate-votes comdexvaloper...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
...
$ comdex query oracle aggregate-prevotes

Or, can filter with voter address

$ comdex query oracle aggregate-prevotes comdexvaloper...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
...
$ comdex query oracle exchange-rates
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
...
$ comdex query oracle exchange-rate ATOM
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
