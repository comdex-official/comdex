...
func (s *IntegrationTestSuite) TestSetHistoraclePricing() {
	...
	s.Require().Equal(medians[0], *types.NewPriceStamp(sdk.MustNewDecFromStr("1.2"), displayDenom, 17))
	s.Require().Equal(medians[1], *types.NewPriceStamp(sdk.MustNewDecFromStr("1.125"), displayDenom, 14))
	s.Require().Equal(medians[2], *types.NewPriceStamp(sdk.MustNewDecFromStr("1.1"), displayDenom, 11))

	medianDeviation, err := app.OracleKeeper.HistoricMedianDeviation(ctx, displayDenom)
	s.Require().NoError(err)

	s.Require().Equal(medianDeviation, types.NewPriceStamp(sdk.MustNewDecFromStr("0.111803398874989476"), displayDenom, 17))
